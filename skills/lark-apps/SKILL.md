...
| 查看某次会话某一轮（turn）的回复消息 / 导出上一轮模型回复（"这一轮回复了什么""上一轮的回复""导出某轮消息"） | 先 `+session-get`（取 `latest_turn.turn_id`）-> `+message-get --turn-id <id>`（仅 user 身份；分页用 `--cursor`） | [`lark-apps-message-get.md`](references/lark-apps-message-get.md) |
